...
Copy `.env.example` to `.env`, run `npm install` and `npm run dev`. Start the existing backend separately on its configured port (default 5000). `VITE_API_BASE_URL=/api` uses the development proxy; set `API_PROXY_TARGET` when the backend uses another port. Use the actual frontend URL printed by Vite (normally http://localhost:5173).
...
`VITE_API_BASE_URL` is the full API prefix including `/api`, for example `https://your-api-host/api`. It is public build-time configuration: rebuild after changing it. Do not place tokens or secrets in VITE variables.

The existing `vercel.json` already rewrites SPA routes to `/index.html`, including direct `/track` refresh. On Vercel set a cross-origin API URL; the local Vite proxy does not exist in production. Do not leave `/api` pointing at the SPA catch-all unless the host has a separate API reverse proxy.
...
- Backend port 5000 was unreachable; live success/status-matrix testing and production CORS/refresh checks remain deployment verification tasks.
